...
cl_parser.add_argument('-a',
                       '--plccarguments',
                       help = 'a string with the plcc arguments you want to use and its values to execute plcc in different ways using plccs command line arguments.')
...
for elem in programm_list:
    ...
    if (elem == 'toLegacyPDB.py'):
        ...
        exec_string = cmd_start + elem + ' -id ' + i_dir + ' -od ' + out_path + cmd_compound
        ...
        exec_string = cmd_start + elem + ' -id ' + dssp_o_dir + ' -od ' + out_path
        log('exec_string ' + exec_string, 'd')
        os.system(exec_string)
        dssp_o_dir = out_path
        ...
        for pdb in os.listdir(pdb_o_dir):
            if (pdb.endswith(".pdb")):
                ...
                plcc = 'java -jar ' + plccJarPath + ' ' + pdb_id + ' -p ' + pdb + ' -d ' + dssp
            ...
            if (os.path.isdir(entry)):
                for gml in os.listdir(entry):
                    if gml.endswith("complex_chains_albelig_CG.gml"):
                        ...
                        if prevGml != '':
                            gmlComparison = cmd_start + elem + ' -i1 ' + out_path + prevGml + ' -i2 ' + out_path + gml + ' -od ' + o_dir
                ...
                if entry.endswith("complex_chains_albelig_CG.gml"):
                    ...
                    if prevGml != '':
                        gmlComparison = cmd_start + elem + ' -i1 ' + out_path + prevGml + ' -i2 ' + out_path + entry + ' -od ' + o_dir
                        os.system(gmlComparison)
                prevGml = entry
