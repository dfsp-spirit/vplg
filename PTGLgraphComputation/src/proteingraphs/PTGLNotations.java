...
public class PTGLNotations {
    ...
    public List<PTGLNotationFoldResult> getResults() {
        if(! this.resultsComputed) {
            ...
            if(Settings.getBoolean("PTGLgraphComputation_B_special_linnot_rules_for_bifurcated_adj_and_red")) {
    ...
    public void computeLinearNotations() {                        
		...
		seqstart = cur;
                ...
                if(isNotBifurcated) {
                    ...
                    Integer[] orientationsSpatOrder = new Integer[fg.getSize()];    // the heading of the vertex in the image (UP or DOWN). This is in the order of spatOrder variable, not the the original vertex list in the SSEList variable.
                    ...
                    List<Integer> parentVertexPosInFG = ProtGraph.computeParentGraphVertexPositionsInFoldingGraph(fgVertexPosInParent, g.getSize());
                    Integer v;
                    ...
                    if(keyposFGIndicesSpatOrder.size() != fg.getSize()) {
                        ...
                        if(keyposFGIndicesSpatOrder.size() != fg.getSize()) {
                            DP.getInstance().e("SSEGRaph", "keyposFGIndicesSpatOrder.size()=" + keyposFGIndicesSpatOrder.size() + ", fg.size=" + fg.getSize() + ".");
                        }
                    }
    ...
    /**
     * Computes all PTGL linear notations for all folds, but does extra rules for ADJ and RED if the graphs are bifurcated. This is what Ina says was used in the original PTGL -- it is NOT done like this in the Perl script by Patrick May though. Most likely different versions existed in the past. You must use the getResults() method to retrieve the results.
     */
    public void computeLinearNotationsExtraRulesForBifurcated() {                        
		...
		seqstart = cur;
                ...
                // BIFURCATED_EXTRA_RULE
                if(! isNotBifurcated) {
                    adjcur = ccVerts.get(0);    // start at vertex 0 (left most of CC)
                }
                ...
                if(isMultiSSETypeGraph) {
                //if(g.getGraphType().equals(ProtGraphs.GRAPHTYPE_STRING_ALBE)) {
                    String startLabel = g.getVertex(adjcur).getLinearNotationLabel();
                    ADJ.append(startLabel);
                    if(adjverbose) {
                        System.out.println("    Start vertex is " + adjcur + ", type:" + g.getVertex(adjcur).getPLCCSSELabel() + ", label=" + startLabel + ".");
                    }
                }
                
                adjstart = adjcur;
                adjvisited.add(adjcur);
                adjpos.add(adjcur);
                //List<Integer> adjdegrees = new ArrayList<>();
                HashMap<Integer, Integer> adjdegrees = new HashMap<>();
                for(Integer key : degrees.keySet()) {
                ...
                // BIFURCATED_EXTRA_RULE
                if( ! isNotBifurcated) {
                    redcur = ccVerts.get(0);    // start at left-most v in cc
                }
                ...
                //#########################################################################
                //############################ KEY impl 2, UNUSED #########################
                //############ Do not use this, it is not a valid KEY notation ############
                //#########################################################################
