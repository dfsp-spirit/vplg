/*
 * This file is part of the Visualization of Protein Ligand Graphs (VPLG) software package.
 *
 * Copyright Tim Schäfer 2012. VPLG is free software, see the LICENSE and README files for details.
 *
 * @author ts
 */

package motifs;

/**
 * Some protein fold motifs. The functions to compute these are in the DBManager class.
 * @author ts
 */
public class Motifs {
    
    public static final String MOTIF__FOUR_HELIX_BUNDLE = "4helix";
    public static final String MOTIF__GLOBIN_FOLD = "globin";
    public static final String MOTIF__UP_AND_DOWN_BARREL = "barrel";
    public static final String MOTIF__IMMUNOGLOBIN_FOLD = "immuno";
    public static final String MOTIF__BETA_PROPELLER = "propeller";
    public static final String MOTIF__JELLY_ROLL = "jelly";
    public static final String MOTIF__UBIQUITIN_ROLL = "ubi";
    public static final String MOTIF__ALPHA_BETA_PLAIT = "plait";
    public static final String MOTIF__ROSSMAN_FOLD = "rossman";
    public static final String MOTIF__TIM_BARREL = "tim";
    public static final String MOTIF__FERREDOXIN_FOLD = "ferre";
    public static final String MOTIF__LIGAND_HELIX = "lighelix";
    public static final String MOTIF__LIGAND_STRAND = "ligstrand";
    
    public static final Integer MOTIFCODE__FOUR_HELIX_BUNDLE = 1;
    public static final Integer MOTIFCODE__GLOBIN_FOLD = 2;
    public static final Integer MOTIFCODE__UP_AND_DOWN_BARREL = 3;
    public static final Integer MOTIFCODE__IMMUNOGLOBIN_FOLD = 4;
    public static final Integer MOTIFCODE__BETA_PROPELLER = 5;
    public static final Integer MOTIFCODE__JELLY_ROLL = 6;
    public static final Integer MOTIFCODE__UBIQUITIN_ROLL = 7;
    public static final Integer MOTIFCODE__ALPHA_BETA_PLAIT = 8;
    public static final Integer MOTIFCODE__ROSSMAN_FOLD = 9;
    public static final Integer MOTIFCODE__TIM_BARREL = 10;
    public static final Integer MOTIFCODE__FERREDOXIN_FOLD = 11;
    public static final Integer MOTIFCODE__LIGAND_HELIX = 12;
    public static final Integer MOTIFCODE__LIGAND_STRAND = 13;
    
}
