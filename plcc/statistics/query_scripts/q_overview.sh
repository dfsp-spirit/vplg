...
psql $PSQL_OPTIONS -c 'SELECT count(foldinggraph_id) from plcc_foldinggraph fg INNER JOIN plcc_graph g ON fg.parent_graph_id = g.graph_id WHERE ( fg.graph_type = 1 ) ;'

echo "Number of beta-PG-based folding graphs:"
psql $PSQL_OPTIONS -c 'SELECT count(foldinggraph_id) from plcc_foldinggraph fg INNER JOIN plcc_graph g ON fg.parent_graph_id = g.graph_id WHERE ( fg.graph_type = 2 ) ;'

echo "Number of albe-PG-based folding graphs:"
psql $PSQL_OPTIONS -c 'SELECT count(foldinggraph_id) from plcc_foldinggraph fg INNER JOIN plcc_graph g ON fg.parent_graph_id = g.graph_id WHERE ( fg.graph_type = 3 ) ;'

echo "Number of alphalig-PG-based folding graphs:"
psql $PSQL_OPTIONS -c 'SELECT count(foldinggraph_id) from plcc_foldinggraph fg INNER JOIN plcc_graph g ON fg.parent_graph_id = g.graph_id WHERE ( fg.graph_type = 4 ) ;'

echo "Number of betalig-PG-based folding graphs:"
psql $PSQL_OPTIONS -c 'SELECT count(foldinggraph_id) from plcc_foldinggraph fg INNER JOIN plcc_graph g ON fg.parent_graph_id = g.graph_id WHERE ( fg.graph_type = 5 ) ;'

echo "Number of albelig-PG-based folding graphs:"
psql $PSQL_OPTIONS -c 'SELECT count(foldinggraph_id) from plcc_foldinggraph fg INNER JOIN plcc_graph g ON fg.parent_graph_id = g.graph_id WHERE ( fg.graph_type = 6 ) ;'
