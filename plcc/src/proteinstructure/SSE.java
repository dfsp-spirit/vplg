...
public class SSE extends SSEGraphVertex implements IDrawableVertex, java.io.Serializable  {
    ...
    private void computeOrientationVector() {
        ...
        int numResForCentroid = Math.min(SettingsOld.getInteger("plcc_I_spatrel_vector_num_res_centroids"), this.getLength() - 1);
    ...
    public Integer angleBetweenThisAnd(SSE otherSSE) {
        ...
        if (angleDeg <= SettingsOld.getInteger("plcc_I_spatrel_max_deg_parallel")) {
            if (SettingsOld.getInteger("plcc_I_debug_level") >= 3) { System.out.println("  [DEBUG LV 3] Assigning parallel orientation"); }
            return SpatRel.PARALLEL;
        } else if (angleDeg < SettingsOld.getInteger("plcc_I_spatrel_min_deg_antip")) {
            if (SettingsOld.getInteger("plcc_I_debug_level") >= 3) { System.out.println("  [DEBUG LV 3] Assigning mixed orientation"); }
            return SpatRel.MIXED;
        } else if (angleDeg <= 180) {
            if (SettingsOld.getInteger("plcc_I_debug_level") >= 3) { System.out.println("  [DEBUG LV 3] Assigning antiparallel orientation"); }
