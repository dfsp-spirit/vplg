...
if($valid_values){    	
	...
	if(file_exists($first_full_file) && file_exists($second_full_file)){
        ...
        while ($file = readdir($handler)) {
          ...
          if ($file != "." && $file != "..") {
		    // find file names like 'results_0_first.txt', where the 0 can be any number.
            if(preg_match('#^(results_)[^\s]*_first(\.(txt))#', $file)) {
              // add to our file array for later use
              $results[] = $file;
            }
			else {
			  //$tableString .= "Skipping file $file.<br>";
			}
           }
        }
		$num_found = count($results);
		$tableString .= "<table><tr><th>File name</th><th>Action</th></tr>";
		foreach($results as $file) {
		  $tableString .= "<tr><td>$file</td><td>Visualize substructure</td></tr>";
		}
		$tableString .= "</table>";
	}
	else {
