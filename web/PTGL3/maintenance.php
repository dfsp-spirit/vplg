		...
		<!-- Intro message -->
		    ...
		    <?php
			  ...
			  if($CHECK_INSTALL_ON_MAINTENANCEPAGE) {
			      ...
			      $db_ok = $db;
			      $tmp_dir_ok = is_writable($TMP_DOWNLOAD_DIR);
			      $tmp_data_dir_ok = is_writable($TMP_DATA_DIR);
		       ...
		       if(check_auth_from_list($_POST['admin_id'], $_POST['admin_token'], $MAINTENANCE_USERS, $MAINTENANCE_TOKENS_MD5)) {
		           ...
		           if($task === "linnot_list") {
					  ...
					  if(! $all_data) { 
					      ...
					      $filename = $TMP_DATA_DIR . "/" . get_linnots_filename($graphtype_int, $notation);
		           ...
		           if($task === "content_stats") {
		               ...
		               $filename = $TMP_DATA_DIR . "/content_data.php";
		               $result_string_stats = '<?php' . "\n";
		               $result_string_stats .= '/* This file is auto-generated by the maintenance page of the PTGL, do not edit it manually. Run an admin maintenance task on the page instead. */' . "\n";
		               
		               $all_queries_ok = TRUE;
		               
		               // get current month and year
		               $last_update_month_and_year = date('F o');
		               $result_string_stats .= get_string_var_line("last_update_month_and_year", $last_update_month_and_year);
		               ...
		               if(is_array($res)) {
		                   $num_protein_chains = $res["cnt"];
		                   $result_string_stats .= get_int_var_line("num_protein_chains", $num_protein_chains);
		               } else {
		               ...
		               // ----- get the number of alpha graphs -----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 1;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_alpha = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_alpha", $num_graphs_alpha);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Alpha PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }
		               
		               // ----- get the number of beta graphs -----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 2;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_beta = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_beta", $num_graphs_beta);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Beta PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }
		               
		               // ----- get the number of albe graphs -----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 3;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_albe = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_albe", $num_graphs_albe);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Albe PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }
		               
		               // ----- get the number of alphalig graphs -----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 4;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_alphalig = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_alphalig", $num_graphs_alphalig);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Alphalig PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }
		               
		               // ----- get the number of betalig graphs -----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 5;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_betalig = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_betalig", $num_graphs_betalig);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Betalig PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }
		               
		               // ----- get the number of albe graphs -----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 6;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_albelig = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_albelig", $num_graphs_albelig);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Albelig PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }
		               
		               
		               // =================================== graphs containing beta barrels ========================
		               
		               // ----- get the number of alpha graphs containing barrels-----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 1 AND g.graph_containsbetabarrel = 1;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_alpha_with_barrel = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_alpha_with_barrel", $num_graphs_alpha_with_barrel);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Alpha barrel PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }
		               
		               // ----- get the number of beta graphs containing barrels -----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 2 AND g.graph_containsbetabarrel = 1;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_beta_with_barrel = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_beta_with_barrel", $num_graphs_beta_with_barrel);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Beta barrel PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }
		               
		               // ----- get the number of albe graphs containing barrels -----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 3 AND g.graph_containsbetabarrel = 1;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_albe_with_barrel = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_albe_with_barrel", $num_graphs_albe_with_barrel);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Albe barrel PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }
		               
		               // ----- get the number of alphalig graphs containing barrels -----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 4 AND g.graph_containsbetabarrel = 1;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_alphalig_with_barrel = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_alphalig_with_barrel", $num_graphs_alphalig_with_barrel);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Alphalig barrel PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }
		               
		               // ----- get the number of betalig graphs containing barrels -----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 5 AND g.graph_containsbetabarrel = 1;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_betalig_with_barrel = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_betalig_with_barrel", $num_graphs_betalig_with_barrel);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Betalig barrel PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }
		               
		               // ----- get the number of albe graphs containing barrels -----
		               $res = array();
		               $res = handle_fixed_query_one_result($db, "SELECT count(g.graph_id) as num_graphs from plcc_graph g WHERE g.graph_type = 6 AND g.graph_containsbetabarrel = 1;", array("num_graphs"));
		               if(is_array($res)) {
		                   $num_graphs_albelig_with_barrel = $res["num_graphs"];
		                   $result_string_stats .= get_int_var_line("num_graphs_albelig_with_barrel", $num_graphs_albelig_with_barrel);
		               } else {
		                   array_push($SHOW_ERROR_LIST, "Albelig barrel PG count database query failed: '" . $res[0] . "'");
		                   $all_queries_ok = FALSE;
		               }		               
