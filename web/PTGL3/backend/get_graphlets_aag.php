<?php
/** This file provides search for AAG graphlets.
 * 
 * 
 * @author Tim Schäfer
 */
...
function get_aag_graphlets_query_string($pdb_id) {
  $query = "SELECT aag.pdb_id, array_to_json(aagg.aa_graphlet_counts) AS graphlets FROM plcc_aa_graphlets aagg INNER JOIN plcc_aagraph aag ON aagg.aagraph_id = aag.aagraph_id WHERE (aag.pdb_id = '$pdb_id');";
  return $query;
}
...
if($valid_values){
	...
	$query = get_aag_graphlets_query_string($pdb_id);
	...
	$tableString .= "<caption> The graphlets of the AAG of PDB $pdb_id </caption>\n";
	$tableString .= "<tr>
    <th>PDB</th>
	...
	if($num_found == 1 && count($res_graphlets) > 0) {
	  ...
	  $tableString .= "<caption> Graphlets details of the AAG of PDB $pdb_id </caption>\n";
